...
@endphp
<form id="form_bulan" method="get" action="{{ route('cekin') }}" class="w-full max-w-sm px-2 py-1 mx-auto border rounded border-black mt-5">
    @csrf
    <input type="hidden" name="idSiswa" id="scannedNumber" value="{{ $id }}-{{ $username }}">
...
<div class="grid grid-cols-7 gap-2 p-2 mt-4 w-full max-w-sm mx-auto border rounded border-black">
    @for ($i = 1; $i <= $lastDayOfMonth; $i++)
        ...
        <div class="border-2 border-blue-400 h-10 rounded-full text-center flex items-center justify-center {{ $isSiswaDate ? 'highlight' : '' }}">
            {{ $i }}
        </div>
    @endfor
</div>
