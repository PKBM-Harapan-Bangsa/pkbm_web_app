...
Toggle modal
        ...
        <section class="w-full flex mt-5">

            <div class="flex-1 p-2 grid sm:grid-cols-2 grid-cols-1 gap-2">
                
            @foreach($spp as $spps)
                <button data-modal-target="default-modal-{{ $spps->id }}" data-modal-toggle="default-modal-{{ $spps->id }}" class="bg-indigo-400 rounded-lg p-2 flex flex-col w-full hover:bg-indigo-700 hover:shadow-xl focus:ring focus:ring-offset-2 focus:ring-indigo-600">
                    <div class="text-center bg-white w-full rounded pb-1 px-1">
                        <p class="font-extrabold text-indigo-600 text-xl">{{ $spps->jenis }}</p>
                        <p class="text-sm bg-indigo-400 rounded text-white">Kelas {{ $spps->kelas }}</p>
                    </div>
